...
npc = length(ens_pc)
for (n in ens_pc){
  if (!dir.exists(paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", n, "PC"))){
    dir.create(paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", n, "PC"))
  ...
  for (seas in 1:4){
    load(file = paste0("~/Documents/LSCE/SWG/NA_slp_sst/pca/predictor_",SEAS[seas],"_4.RData"))
    SD = PCS
    load(file = paste0("~/Documents/LSCE/SWG/slp_PC_diagnosis/", n, "PC/predictor_",SEAS[seas],"_1.RData"))
    PCS = cbind(PCS, SD)
    colnames(PCS) = paste0("PC", 1:ncol(PCS))
    PCS = as.data.frame(PCS)
    save(PCS, file = paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", n, "PC/predictor_",SEAS[seas],"_7.RData"))
  }
}

#### estimation for each case ####
source(file = "funtion/fun_estimation_t2m.R")
## 1979 - 1998
for (n in ens_pc){
  for (k in 7)(
    fun_estimation(predictor = paste0('~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/', n, 'PC/predictor_'),
                   NUM = k,
                   input.tmean = "~/Documents/LSCE/SWG/tmean_48_50_N_01_04_E_1979_2017.RData",
                   output.name = paste0('~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/', n, 'PC/SWG_ERAI_ESD_tmean_'),
                   year.begin = 1979,
                   year.end = 1998)
  )
}
...
#### simulation ####
source(file = "funtion/fun_simulation_t2m.R")

for (n in ens_pc){
  for (k in 7){
    cat(n, "\n")
    fun_simulation(predictor = paste0('~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/', n, 'PC/predictor_'),
                   parameter = paste0('~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/', n, 'PC/SWG_ERAI_ESD_tmean_'),
                   NUM = k, 
                   type = "nat",
                   output = paste0('~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/', n, 'PC/'), 
                   y1 = 1999, y2 = 2017,
...
for (k in 1:npc){
  i = ens_pc[k]
  dir.create(path = paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", i, "PC/SIMU"))
  dir.create(path = paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", i, "PC/SIMU/nat"))
  dir.create(path = paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", i, "PC/SIMU/rea"))
}

for (k in 1:npc){
  pc = ens_pc[k]
  w = "nat"
  load(paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", pc, "PC/t2m_mean_sd_1999_2017_", w, "_7.RData"))
  range(mean)
  range(sd)
  ...
  for(i in 1:100){
    ...
    filesimu = paste('~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/', pc, 'PC/SIMU/', w, '/SIMU_SWG_tmean_1999_2017_', w, '_', 7, '_run_', NUM, '.RData', sep="")
...
for (k in 1:npc){
  ...
  for (i in 1:100){
    NUM=cbind(formatC(i, digits = 0, width = 3, format = "f", flag = "0"))
    load(paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", pc, "PC/SIMU/nat/SIMU_SWG_tmean_1999_2017_nat_", 7, "_run_", NUM, ".RData"))
    for (seas in 1:4){
      ...
      save(DATE_seas, Sample_seas, file = paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", pc, "PC/SIMU/nat/SIMU_tmean_1999_2017_nat_7_", SEAS[seas], "_run_", NUM, ".RData"))
    }
  }
}
...
#### Distribution ####
source(file = "funtion/CramerVonMisesTwoSamples.R")
...
for (k in 1:npc){
  ...
  for (i in 1:100){
    NUM=cbind(formatC(i, digits = 0, width = 3, format = "f", flag = "0"))
    load(paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", pc, "PC/SIMU/nat/SIMU_SWG_tmean_1999_2017_nat_7_run_", NUM, ".RData"))
    Sample = Sample[,1]
...
p <- ggplot(res.melt) + geom_boxplot(aes(x=variable, y=value)) + theme_bw() + 
  labs(x="slp - number of PC", y="Cramer von Mises", 
       title = paste0("Cramer von Mises - slp_PC_sst_SD - 100 simulations (1999-2017)"))
print(p)
dev.print(pdf, file = "~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/Image/boxplot_Cramer-von_Mises_with_slp_PC.pdf", width = 7, height = 7)

pva.melt = melt(pva)
p <- ggplot(pva.melt) + geom_boxplot(aes(x=variable, y=value)) + theme_bw() + 
  labs(x="slp - number of PC", y="p value", 
       title = paste0("p-value - slp_PC_sst_SD - 100 simulations (1999-2017)"))
print(p)
dev.print(pdf, file = "~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/Image/boxplot_pvalue_with_slp_PC.pdf", width = 7, height = 7)
...
for (seas in 1:4){
  ...
  for (k in 1:npc){
    ...
    for (i in 1:100){
      NUM=cbind(formatC(i, digits = 0, width = 3, format = "f", flag = "0"))
      load(paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", pc, "PC/SIMU/nat/SIMU_tmean_1999_2017_nat_7_", SEAS[seas], "_run_", NUM, ".RData"))
  ...
  p <- ggplot(res.melt) + geom_boxplot(aes(x=variable, y=value)) + theme_bw() + 
    labs(x="slp - number of PC", y="Cramer von Mises", 
         title = paste0("Cramer von Mises - slp_PC_sst_SD - 100 simulations (", season[seas], " 1999-2017)"))
  print(p)
  dev.print(pdf, file = paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/Image/boxplot_Cramer-von_Mises_with_slp_PC_", season[seas], ".pdf"), width = 7, height = 7)
  
  pva.melt = melt(pva)
  p <- ggplot(pva.melt) + geom_boxplot(aes(x=variable, y=value)) + theme_bw() + 
    labs(x="slp - number of PC", y="p value", 
         title = paste0("p-value - slp_PC_sst_SD - 100 simulations (", season[seas], " 1999-2017)"))
  print(p)
  dev.print(pdf, file = paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/Image/boxplot_pvalue_with_slp_PC_", season[seas], ".pdf"), width = 7, height = 7)
}
...
for (k in 1:npc){
  ...
  load(paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", i, "PC/t2m_mean_sd_1999_2017_nat_7.RData"))
  cat(range(mean),'\n')
  cat(range(sd),'\n')
  ...
  load(paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/", i, "PC/t2m_mean_sd_1999_2017_rea_7.RData"))
  cat(range(mean),'\n')
  cat(range(sd),'\n')
  m = as.data.frame(matrix(NA, nrow = nrow(mean), ncol = 2))
  colnames(m) = c("mean", "sd")
...
DATE = as.Date((0:(n-1)), origin = "1999-01-01")
d$date = DATE
save(d, file = "~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/d_intensity.RData")
...
npc = length(ens_pc)
ncase = npc+1
load(file = "~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/d_intensity.RData")
...
for (year in 1999:2017){
  ...
  print(p)
  dev.print(pdf, file=paste0("~/Documents/LSCE/SWG/slp_PC_sst_SD_diagnosis/Image/d_intensity_tmean_summer_", year, ".pdf"), width = 7, height=7)
  print(year)
}
