...
fun_qqplot(Obs, Sample, main = "precip")

## load all simulations
Sample_all = c()
for (i in 1:100){
  NUM=cbind(formatC(i, digits = 0, width = 3, format = "f", flag = "0"))
  load(paste0("~/Documents/LSCE/SWG/tp/case_2/SIMU/SIMU_SWG_precip_run_", NUM, ".RData"))
  Sample_all = c(Sample_all, Sample)
}

fun_qqplot(Obs, Sample_all, main = "tmean - 100 simulations - case_3")
dev.print(pdf, file = "~/Documents/LSCE/SWG/tp/Images/qqplot_case_3.pdf", width = 7, height = 7)
