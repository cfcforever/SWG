...
for (seas in 1:4){
  ...
  for (ncase in c(29,48,57,63)){
    # load one Sample (see k and NUM)
    load(paste0("~/Documents/LSCE/SWG/NA/tmean/SIMU/", ncase, "/SIMU_tmean_", SEAS[seas], "_run_001.RData"))
    Sample = Sample_seas
    ...
    color = c("black", "red")
    {
      ...
      p = p + xlab("day")
      p = p + ggtitle(paste0("acf - Obs vs Sample_", k, " for ", case_name[ncase]))
      print(p)
    }
    dev.print(pdf, file = paste0("~/Documents/LSCE/SWG/NA/Image/acf/acf_", SEAS[seas],"_case_", ncase, ".pdf"), width = 9, height = 9)
